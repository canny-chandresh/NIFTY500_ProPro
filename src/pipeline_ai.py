...
from error_logger import RunLogger                    # your existing structured logger
from config_guard import config_diff                  # tracks config hash & change keys
from ai_policy import build_context                   # builds market context (VIX/regime/etc.)
from model_selector import choose_and_predict_full    # ranks symbols & returns dataframe + model tag
from risk_manager import apply_guardrails             # ATR stops/targets, sanity & bounds
from atr_tuner import update_from_metrics             # adjusts ATR multipliers from recent metrics
from portfolio import optimize_weights                # HRP/equal/risk parity + sector caps/turnover
from validator import validate_orders_df              # order schema sanity
...
# Telegram (safe fallback if module not available)
try:
...
def _paper_fill_df(picks: pd.DataFrame, engine: str, default_mode: str = "swing") -> pd.DataFrame:
    """
    Convert a scored picks dataframe into paper-trade rows with
    Entry/Target/SL/size placeholders when missing.
    """
    ...
    for col in ("Symbol","Entry","Target","SL","proba"):
        if col not in d.columns:
            if col == "Symbol": d["Symbol"] = ""
            elif col == "Entry": d["Entry"] = d.get("Close", 0.0)
...
def run_auto_and_algo_sessions(top_k_auto: int | None = None, top_k_algo: int | None = None) -> tuple[int,int]:
    ...
    End-to-end pipeline:
      - SLI & alert
      - Corporate actions adjust (if bhavcopy/raw present)
      - News ingest & feature merge
      - Context build (VIX/regime)
      - Model selection -> ranked picks
      - Guardrails, eligibility, calendars
      - Portfolio sizing with sector caps/turnover
      - Orders (paper) + validation
      - Explainability (light)
      - Registry snapshot
      - Telegram notify
      - Metrics snapshot + ATR tuner
      - DL shadow training with kill-switch
    """
    _ensure_dirs()
    ...
    with logger.capture_all("pipeline_run", swallow=True):
        ...
        with logger.section("data/sli"):
            try:
                ...
                try:
                    alert_if_bad(sli, tg_send=lambda header: send_recommendations(header=header))
                except Exception:
                    pass
            except Exception as e:
                print("[sli] error:", e)

        # 0b) Corporate actions normalization (if present)
        ...
        # 4) Guardrails (ATR sizing, sanity checks, options sanity)
        with logger.section("guardrails/AUTO"):
            auto_df = apply_guardrails(raw_df)

        # 4b) Eligibility gates & calendars (earnings/ex-dates; macro-day thinning)
        with logger.section("eligibility/calendars"):
            try:
                auto_df = apply_gates(
                    auto_df,
                    min_liq_value=float(CONFIG.get("market", {}).get("min_liquidity_value", 2_00_00_000))
                )
                # block symbols near events (earnings/ex-div) within +/-1 day
                today = pd.Timestamp.utcnow().tz_localize("UTC")
                keep_mask = []
                for s in auto_df["Symbol"]:
                    keep_mask.append(not policy_window_block(s, today, pre_days=1, post_days=0))
                auto_df = auto_df.loc[keep_mask].reset_index(drop=True)

                # macro impact days → thin book by half
                if macro_block(today):
                    auto_df = auto_df.head(max(1, len(auto_df)//2))
            except Exception as e:
                print("[eligibility/calendars] error:", e)

        # 5) ALGO leftovers (for learning / exploration)
        ...
        # 6) Portfolio sizing (HRP etc.) with sector caps + turnover
        with logger.section("portfolio/size"):
            last_w_path = os.path.join(DL, "last_weights.csv")
            lastW = None
            if os.path.exists(last_w_path):
                try:
                    lastW = pd.read_csv(last_w_path)
                except Exception:
                    lastW = None
            ...
            try:
                pd.concat(
                    ...
                    ignore_index=True
                ).to_csv(last_w_path, index=False)
            except Exception as e:
                print("[portfolio] persist last_weights error:", e)

        # 7) Orders (paper) & validate, persist to datalake/paper_trades.csv
        ...
        # 8) Explainability (simple: SHAP/permutation/variance fallback)
        ...
        # 8b) Registry snapshot (store context hash/metadata)
        ...
        # 8c) Deep Learning shadow training with kill-switch
        with logger.section("dl/shadow_train"):
            try:
                from dl_models.master_dl import DeepLearningTrainer
                from dl_kill_switch import should_train_dl, record_result
                ks_cfg = CONFIG.get("dl_killswitch", {})
                ok, why = should_train_dl(ks_cfg)
                logger.add_meta(dl_kill_switch=why)
                if ok:
                    dl = DeepLearningTrainer(window_days=90)
                    dl_metrics = dl.train()
                    logger.add_meta(dl=dl_metrics)
                    wr = float(dl_metrics.get("win_rate_est", 0.0))
                    state = record_result(wr, ks_cfg)
                    logger.add_meta(dl_kill_state=state)
                else:
                    logger.add_meta(dl_skipped=True)
            except Exception as e:
                print("[dl] error:", e)
        ...
        # 11) ATR tuner update (adapts ATR MULTs over time)
        with logger.section("atr_tuner/update"):
            try:
                update_from_metrics(ctx)
            except Exception as e:
                print("[atr_tuner] error:", e)
