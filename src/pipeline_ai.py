...
def _run_block(purpose: str, top_k: int):
    ...
    # Decide routing mode and limits
    if purpose == "auto":
        ...
        # only allow LIVE if flag says allowed AND global enable_algo_live True AND dry_run False
    ...
    for _, r in picks.head(max_trades).iterrows():
        ...
        # clip per-trade size for safety (policy already sized)
        size_pct = min(size_pct, per_cap)

        # naive qty proxy (paper): you can wire real capital → qty later
        qty = max(1, int(1 * size_pct * 10))  # small integer qty
        meta = {
