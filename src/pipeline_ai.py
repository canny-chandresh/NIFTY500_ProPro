...
def _append_csv(path: str, df: pd.DataFrame):
    os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
    (df.to_csv(path, index=False) if not os.path.exists(path) else df.to_csv(path, mode="a", index=False, header=False))
...
def run_auto_and_algo_sessions(top_k_auto: int | None = None, top_k_algo: int | None = None) -> tuple[int,int]:
    ...
    with logger.capture_all("pipeline_run", swallow=True):
        ...
        # 0b) Corporate actions normalization (if any raw uploads exist)
        ...
        with logger.section("portfolio/size"):
            ...
            try:
                auto_df = optimize_weights(auto_df,
            ...
            try:
                algo_df = optimize_weights(algo_df,
            ...
            try:
                pd.concat([auto_df[["Symbol","size_pct"]].assign(engine="AUTO"),
                           algo_df[["Symbol","size_pct"]].assign(engine="ALGO")], ignore_index=True)\
                  .to_csv(last_w_path, index=False)
            except Exception as e: print("[portfolio] persist last_weights error:", e)
        ...
        # 8) Explainability (stubbed to run; replace model to get true SHAP)
        ...
        # 8b) Registry record (context-only until real model object is stored)
        ...
        with logger.section("snapshot/metrics"):
            snap = {"when_utc": _utcnow(),"model_used": model_tag,
                    "auto_count": int(len(auto_orders)),"algo_count": int(len(algo_orders)),
                    "config_changed_keys": list(cfg_diff.get("changed", {}).keys())}
            _log_json(os.path.join(MET_DIR, "last_run.json"), snap); logger.add_meta(**snap)
