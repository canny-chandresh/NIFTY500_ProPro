...
def reward_from_stats(stats: dict) -> float:
    """
    Scalar reward in [0,1] from recent performance.
    Components:
      - win_rate (primary)
    ...
    dd   = float(stats.get("max_drawdown", 0.0))       # positive (e.g., 0.08 for -8%)
    vol  = float(stats.get("ret_vol", 0.0))            # std dev of per-trade returns

    # Smooth pnl (less sensitive when n is small), clip via tanh
    pnl_adj = math.tanh(pnl / max(1.0, math.sqrt(n)))

    # Combine
    score = 0.55 * wr + 0.25 * pnl_adj - 0.10 * dd - 0.10 * vol

    # squash to [0,1] with soft margin around 0.5
